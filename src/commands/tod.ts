...
import { getRandomTod } from "../utils/promptStore";
import { getRandomTruth, getRandomDare } from "../utils/promptStore";

export const command = new SlashCommandBuilder()
  .setName("tod")
  .setDescription("Get a random Truth or Dare prompt");
...
function attachCollector(message: Message, originUserId: string) {
  ...
  collector.on("collect", async (buttonInteraction: ButtonInteraction) => {
    ...
    if (buttonInteraction.customId === "tod_truth") {
      ...
      const rolled = getRandomTod();
      content = `🎲 **${rolled.kind}:** ${rolled.prompt}`;
    }
